 ...
 */
public class BufferedEmittingPublisherReentrant<T> implements Flow.Publisher<T> {

    private final AtomicReference<State> state = new AtomicReference<>(State.READY_TO_EMIT);
    private final ConcurrentLinkedQueue<T> buffer = new ConcurrentLinkedQueue<>();
    private final EmittingPublisherReentrant<T> emitter = new EmittingPublisherReentrant<>();
    ...
    protected BufferedEmittingPublisherReentrant() {
    }

    /**
     * Create new {@link BufferedEmittingPublisherReentrant}.
     *
     * @param <T> type of emitted item
     * @return new instance of BufferedEmittingPublisher
     */
    public static <T> BufferedEmittingPublisherReentrant<T> create() {
        return new BufferedEmittingPublisherReentrant<T>();
    }
     ...
     * {@link BufferedEmittingPublisherReentrant#emit(Object)} throws {@link IllegalStateException} after calling fail.
     *
     ...
     * {@link BufferedEmittingPublisherReentrant#emit(Object)} throws {@link IllegalStateException} after calling complete.
     */
     ...
     * Nothing else can be sent downstream after calling {@link BufferedEmittingPublisherReentrant#completeNow()}.
     * {@link BufferedEmittingPublisherReentrant#emit(Object)} throws {@link IllegalStateException} after calling completeNow.
     */
     ...
     * Returns {@code true} right after calling {@link BufferedEmittingPublisherReentrant#completeNow()}
     * but after calling {@link BufferedEmittingPublisherReentrant#complete()} returns
    ...
    private enum State {
        READY_TO_EMIT {
            ...
            @Override
            <T> int emit(BufferedEmittingPublisherReentrant<T> publisher, T item) {
            ...
            @Override
            <T> void drain(final BufferedEmittingPublisherReentrant<T> publisher) {
        ...
        abstract <T> int emit(BufferedEmittingPublisherReentrant<T> publisher, T item);

        abstract <T> void drain(BufferedEmittingPublisherReentrant<T> publisher);
