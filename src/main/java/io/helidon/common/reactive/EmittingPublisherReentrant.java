 ...
 */
public class EmittingPublisherReentrant<T> implements Flow.Publisher<T> {
     ...
     */
    public static <T> EmittingPublisherReentrant<T> create() {
        return new EmittingPublisherReentrant<>();
    }
    ...
    private enum State {
        INIT {
            @Override
            <T> boolean emit(EmittingPublisherReentrant<T> publisher, T item) {
                return false;
            }
