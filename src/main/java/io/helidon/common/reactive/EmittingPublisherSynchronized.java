 ...
 */
public class EmittingPublisherSynchronized<T> implements Flow.Publisher<T> {
     ...
     */
    public static <T> EmittingPublisherSynchronized<T> create() {
        return new EmittingPublisherSynchronized<>();
    }
    ...
    private enum State {
        INIT {
            @Override
            <T> boolean emit(EmittingPublisherSynchronized<T> publisher, T item) {
                return false;
            }
