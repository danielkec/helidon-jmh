 ...
 * {@link EmittingPublisherTTAS} doesn't have any buffering capability and propagates backpressure
 * directly by returning {@code false} from {@link EmittingPublisherTTAS#emit(Object)} in case there
 * is no demand, or {@code cancel} signal has been received.
 * <p>
 * For publishing with buffering in case of backpressure use {@link BufferedEmittingPublisherTTAS}.
 * </p>
 *
 * <p>
 ...
 */
public class EmittingPublisherTTAS<T> implements Flow.Publisher<T> {
     ...
     */
    public static <T> EmittingPublisherTTAS<T> create() {
        return new EmittingPublisherTTAS<>();
    }
        ...
        subscriber.onSubscribe(new Flow.Subscription() {
            ...
            public void cancel() {
                if (state.compareAndSet(State.INIT, State.CANCELLED)
                    ...
                    cancelCallback.run();
                    EmittingPublisherTTAS.this.subscriber = null;
                }
            }
    ...
    private enum State {
        INIT {
            @Override
            <T> boolean emit(EmittingPublisherTTAS<T> publisher, T item) {
                return false;
            }
        ...
        abstract <T> boolean emit(EmittingPublisherTTAS<T> publisher, T item);
